...
contract BondDepo is IBondDepository, UserTermsKeeper {
    ...
    )
  {
     ...
     */
    payout_ =
      ((_amount * 10**(2 * req.decimals())) / price) /
  ...
  function _tune(uint256 _id, uint48 _time) internal {
    ...
    if (_time >= meta.lastTune + meta.tuneInterval) {
      ...
      uint256 capacity = market.capacityInQuote
        ? ((market.capacity * (10**(2 * req.decimals()))) / price) /
      ...
      uint256 newControlVariable = uint256(
  ...
  function create(
     ...
     */
    uint256 targetDebt = uint256(
      _booleans[0]
        ? ((_market[0] * (10**(2 * req.decimals()))) / _market[1]) /
          10**decimals
        : _market[0]
    );

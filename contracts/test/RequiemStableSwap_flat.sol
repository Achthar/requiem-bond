...
library RequiemStableSwapLib {
  ...
  function swap(
    ...
    uint256 y = _getY(
      ...
      i,
      j,
      normalizedBalances[i] + (inAmount * self.tokenMultipliers[i]),
      normalizedBalances
    );
  ...
  function onSwapGivenIn(
    ...
    require(
      self.balances[i] + inAmount <=
        IERC20(self.pooledTokens[i]).balanceOf(address(this)),
      "input"
    );
    uint256[] memory normalizedBalances = _xp(self);
    ...
    uint256 _adminFee = (dy_fee * self.adminFee) /
      FEE_DENOMINATOR /
      self.tokenMultipliers[j];
  ...
  function onSwapGivenOut(
    ...
    uint256 _amountOutInclFee = FullMath.mulDiv(
      outAmount,
      FEE_DENOMINATOR,
    ...
    uint256 y = normalizedBalances[j] -
      (_amountOutInclFee * self.tokenMultipliers[j]);
  ...
  function flashLoan(
    ...
    for (uint256 i = 0; i < tokens.length; ++i) {
      ...
      RequiemErrors._require(
        token > previousToken,
        token == IERC20(address(0)) ? Errors.ZERO_TOKEN : Errors.UNSORTED_TOKENS
      );
      previousToken = token;
  ...
  function removeLiquidityImbalance(
    ...
    burnAmount =
      (burnAmount + 1) *
      (FEE_DENOMINATOR - _calculateCurrentWithdrawFee(self, msg.sender)); //In case of rounding errors - make it unfavorable for the "attacker"
  ...
  /// VIEW FUNCTIONS
  function getAPrecise(SwapStorage storage self)
    external
    view
    returns (uint256)
  {
   ...
   * scaled up by 1e18
   */
  function getVirtualPrice(SwapStorage storage self)
    external
    view
    returns (uint256)
  {
  ...
  function getAdminBalance(SwapStorage storage self, uint256 index)
    external
    view
    returns (uint256)
  {
  ...
  function calculateSwap(
    ...
    uint256[] memory normalizedBalances = _xp(self);
    uint256 newInBalance = normalizedBalances[inIndex] +
      (inAmount * self.tokenMultipliers[inIndex]);
    uint256 outBalance = _getY(
      self,
      inIndex,
      outIndex,
      newInBalance,
    ...
    uint256 outAmount = (normalizedBalances[outIndex] - outBalance) /
      self.tokenMultipliers[outIndex];
  ...
  function calculateSwapGivenOut(
    ...
    uint256 newOutBalance = normalizedBalances[outIndex] -
      (_amountOutInclFee * self.tokenMultipliers[outIndex]);
    // switch index on regulat _getY function
    uint256 inBalance = _getY(
      self,
      outIndex,
      inIndex,
      newOutBalance,
   ...
   */
  function _getAPrecise(SwapStorage storage self)
    internal
    view
    returns (uint256)
  {
    ...
    if (self.futureA > self.initialA) {
      return
        self.initialA +
        ((self.futureA - self.initialA) *
          (block.timestamp - self.initialATime)) /
   ...
   */
  function _xp(uint256[] memory balances, uint256[] memory rates)
    internal
    pure
    returns (uint256[] memory)
  {
  ...
  function _xp(SwapStorage storage self)
    internal
    view
    returns (uint256[] memory)
  {
  ...
  function _calculateRemoveLiquidity(
    ...
    uint256 feeAdjustedAmount = (amount *
      (FEE_DENOMINATOR - _calculateCurrentWithdrawFee(self, account))) /
      FEE_DENOMINATOR;
  ...
  function _calculateRemoveLiquidityOneToken(
    ...
    dy =
      (dy * (FEE_DENOMINATOR - _calculateCurrentWithdrawFee(self, account))) /
      FEE_DENOMINATOR;
  }

  function _feePerToken(SwapStorage storage self)
    internal
    view
    returns (uint256)
  {
    ...
    if (self.defaultWithdrawFee == 0) {
      ...
      if ((toMint + currentBalance) * self.defaultWithdrawFee != 0) {
        self.withdrawFeeMultiplier[user] =
          (((currentBalance * currentFee) +
            (toMint * self.defaultWithdrawFee)) * (FEE_DENOMINATOR)) /
   ...
   */
  function _calculateCurrentWithdrawFee(SwapStorage storage self, address user)
    internal
    view
    returns (uint256)
  {
  ...
  function _doTransferIn(IERC20 token, uint256 amount)
    internal
    returns (uint256)
  {
  ...
  IRequiemStableSwap
{
  ...
  function initialize(
    ...
    for (uint256 i = 0; i < numberOfCoins; i++) {
      ...
      require(
        _decimals[i] <= RequiemStableSwapLib.POOL_TOKEN_COMMON_DECIMALS,
        "invalidDecimals"
      );
      rates[i] =
        10**(RequiemStableSwapLib.POOL_TOKEN_COMMON_DECIMALS - _decimals[i]);
  ...
  function onSwap(
    ...
    swapStorage.onSwap(
      tokenIndexes[tokenIn],
      tokenIndexes[tokenOut],
      amountIn,
      amountOut,
      to
    );
  ...
  function getTokenIndex(address token)
    external
    view
    override
    returns (uint8 index)
  ...
  function getTokenPrecisionMultipliers()
    external
    view
    returns (uint256[] memory)
  {
  ...
  function getAdminBalance(uint8 index)
    external
    view
    override
    returns (uint256)
  ...
  function calculateSwapGivenOut(
    ...
    return
      swapStorage.calculateSwapGivenOut(
        tokenIndexes[tokenIn],
        tokenIndexes[tokenOut],
        amountOut
      );
